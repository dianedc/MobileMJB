...
public class TaskSpinnerAdapter extends ArrayAdapter<TaskClass> {
    private Context context;
    private List<TaskClass> values;

    public TaskSpinnerAdapter(@NonNull Context context, List<TaskClass> values) {
    ...
    public View customSpinnerView(final int position, @Nullable View myView, @NonNull ViewGroup parent) {
        ...
        textView_id.setText(String.valueOf(values.get(position).getTaskID()));
        textView_lastname.setText(values.get(position).getTaskname());
        return customView;
